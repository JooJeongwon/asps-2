...
<body>
  ...
  <section>
    ...
    <form id="profile-form">
      <div class="grid">
        ...
        <label>기본 실행 단계<select name="defaultMode"><option>DRAFT_ONLY</option><option>SUGGEST</option><option>SCORE</option><option>SAVE</option><option>FULL_AUTO</option></select></label>
  ...
  <script>
    const state = { me: null, jobs: [] };
    const $ = (selector) => document.querySelector(selector);
    const escapeHtml = (value) => String(value ?? '').replace(/[&<>"']/g, (char) => ({ '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#39;' })[char]);
    const setMessage = (message, kind) => { const node = $('#message'); node.textContent = message || ''; node.className = kind || ''; };
    const api = async (path, options) => {
      const init = options || {};
      const response = await fetch(path, Object.assign({}, init, { headers: Object.assign({ 'content-type': 'application/json' }, init.headers || {}) }));
    ...
    document.addEventListener('click', async (event) => {
      ...
      if (profileId) { if (!confirm('이 profile을 비활성화할까요?')) return; try { await api('/api/me/automation-profiles/' + encodeURIComponent(profileId), { method: 'DELETE' }); setMessage('Profile을 비활성화했습니다.', 'ok'); await load(); } catch (error) { setMessage(error.message, 'error'); } return; }
