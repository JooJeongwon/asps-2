  ...
  <script>
    ...
    const renderJobs = () => {
      ...
      $('#jobs').innerHTML = state.jobs.map((job) => {
        ...
        const retry = job.lastErrorCode !== 'AI_RESULT_AMBIGUOUS' && ['FAILED_RETRYABLE', 'FAILED_FINAL', 'AUTH_REQUIRED'].includes(job.status) ? '<button class="secondary" data-retry-job="' + escapeHtml(job.id) + '" type="button">실패 단계 재시도</button>' : '';
        const actions = job.status === 'SAVED' || job.status === 'CANCELLED' || job.lastErrorCode === 'AI_RESULT_AMBIGUOUS' ? '' : '<button class="secondary" data-job-action="SUGGEST" data-job-id="' + escapeHtml(job.id) + '" type="button">AI 제안</button><button class="secondary" data-job-action="SCORE" data-job-id="' + escapeHtml(job.id) + '" type="button">AI 채점</button><button data-job-action="SAVE" data-job-id="' + escapeHtml(job.id) + '" type="button">저장</button>';
    ...
    const load = async () => { try { const values = await Promise.all([api('/api/me'), api('/api/jobs?limit=50'), api('/api/auth/csrf')]); state.me = values[0]; state.jobs = values[1]; state.csrf = values[2].token; render(); } catch (error) { setMessage(error.message, 'error'); } };
